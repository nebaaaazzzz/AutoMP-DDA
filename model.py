...
import dgl
from gtn.gtn import GTN
from gtn.fast_gtn import FastGTNs
...
class Model(nn.Module):
    ...
    def __init__(self, etypes, ntypes, in_feats, hidden_feats, num_heads, dropout, num_nodes, use_gtn=False,
                 gtn_channels=1, gtn_layers=1, gtn_type='gtn'):
        ...
        if self.use_gtn:
            ...
            if self.gtn_type == 'fast_gtn':
                    # Create a dummy args object for FastGTN
                class FastGTNArgs:
                    pass
                fg_args = FastGTNArgs()
                ...
                self.gtn = GTN(num_edge=num_edge,
                                ...
                                w_out=w_out,
                                num_nodes=num_nodes,
                                num_layers=self.gtn_L,
                                )
            # projection from (C * w_out) -> hidden_feats (we set w_out = hidden_feats)
            if self.gtn_type != 'fast_gtn':
                 self.gtn_proj = nn.Linear(self.gtn_C * hidden_feats, hidden_feats)
                 nn.init.xavier_uniform_(self.gtn_proj.weight)
            else:
    ...
    def forward(self, g, x, mdrug, mdis):
        ...
        if self.use_gtn and (mdrug is None or mdis is None):
            ...
            if self.gtn is None:
                 raise ValueError("GTN module should be initialized in __init__")
            ...
            if self.gtn_type == 'fast_gtn':
                ...
                H_gtn = self.gtn(A, X_homo, num_nodes=N, eval=True)
                # project GTN output to hidden_feats
                H_proj = self.gtn_proj(H_gtn)
